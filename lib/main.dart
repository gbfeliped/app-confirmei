import 'package:confirmeiapp/pages/dashboard/dashboard_page.dart';
import 'package:confirmeiapp/pages/dashboard/requests_item.dart';
import 'package:confirmeiapp/pages/login_page.dart';
import 'package:flutter/material.dart';

void main() {
  runApp(const ConfirmeiApp());
}

Map<int, Color> color = {
  50: Color.fromRGBO(136, 14, 79, .1),
  100: Color.fromRGBO(136, 14, 79, .2),
  200: Color.fromRGBO(136, 14, 79, .3),
  300: Color.fromRGBO(136, 14, 79, .4),
  400: Color.fromRGBO(136, 14, 79, .5),
  500: Color.fromRGBO(136, 14, 79, .6),
  600: Color.fromRGBO(136, 14, 79, .7),
  700: Color.fromRGBO(136, 14, 79, .8),
  800: Color.fromRGBO(136, 14, 79, .9),
  900: Color.fromRGBO(136, 14, 79, 1),
};

class ConfirmeiApp extends StatefulWidget {
  const ConfirmeiApp({key}) : super(key: key);

  @override
  _ConfirmeiAppState createState() => _ConfirmeiAppState();
}

class _ConfirmeiAppState extends State<ConfirmeiApp> {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Flutter Demo',
      theme: ThemeData(
        primarySwatch: MaterialColor(0xFF5F7AF7, color),
      ),
      initialRoute: '/dashboard',
      routes: {
        '/': (context) => LoginPage(),
        //'/dashboard': (context) => RequestItem(),
        '/dashboard': (context) => DashboardPage(),
      },
    );
  }
}
